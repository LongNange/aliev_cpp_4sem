...
//
void print_world(point world[][WORLD_HEIGHT])
{
    unsigned int i, j;
    for (i = 0; i < WORLD_WIDTH; i++) {
        for (j = 0; j < WORLD_HEIGHT; j++) {
            if (world[i][j].is_live == 2)
            {
                cout << '*';
            }
            else if (world[i][j].is_live == 1)
            {
                cout << '+';
            }
            else
...
unsigned int get_live_count(point world[][WORLD_HEIGHT])
{
    ...
    for (i = 0; i < WORLD_WIDTH; i++)
    {
        for (j = 0; j < WORLD_HEIGHT; j++)
        {
            if (world[i][j].is_live == 1 || world[i][j].is_live == 2)
            {
                count++;
            }
        }
...
unsigned int count_live_neighbors(point world[][WORLD_HEIGHT], unsigned int x, unsigned int y)
{
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        if (world[_x][_y].is_live == 1 || world[_x][_y].is_live == 2)
        {
            count++;
        }
    }
